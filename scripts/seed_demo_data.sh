...
# Two calls per client, not one. POST /clients writes the clients row; invoices read the
# billing address out of client_addresses, which that call does not touch. Without the
# second call the first invoice fails with "Client billing address is required" and,
# because of set -e, every invoice, payment, expense and estimate after it silently
# never happens — the account ends up with clients and items and nothing else.
new_client() { # <name> <email> <phone> <line1> <city> <state> <pin>
  local id
  id=$(auth -X POST "$BASE/clients" -d "{\"name\":\"$1\",\"email\":\"$2\",\"phone\":\"$3\",\"address\":\"$4\",\"city\":\"$5\",\"state\":\"$6\",\"pincode\":\"$7\",\"company_id\":$COMPANY_ID}" \
       | grep -o '"client_id":[0-9]*' | grep -o '[0-9]*')
  auth -X POST "$BASE/clients/$id/address" -d "{\"type\":\"billing\",\"name\":\"$1\",\"line1\":\"$4\",\"city\":\"$5\",\"state\":\"$6\",\"postal_code\":\"$7\",\"country\":\"India\",\"phone\":\"$3\",\"email\":\"$2\"}" >/dev/null
  echo "$id"
}
# The company is registered in Delhi, so a Delhi client is billed CGST+SGST and an
# out-of-state client IGST. Both are seeded deliberately — with only out-of-state
# clients the GST report's CGST and SGST columns are always zero.
C1=$(new_client "Aarav Enterprises"     "accounts@aaravent.in"          9812345678 "22 MG Road"          "Bengaluru" "Karnataka"  560001)
C2=$(new_client "Priya Retail Store"    "priya.store@example.in"        9823456789 "5 Linking Road"      "Mumbai"    "Maharashtra" 400050)
C3=$(new_client "Kiran Fabrics Pvt Ltd" "purchase@kiranfabrics.example" 9834567890 "88 Anna Salai"       "Chennai"   "Tamil Nadu" 600002)
C4=$(new_client "Naveen Garments"       "naveen@navgarments.example"    9845678901 "31 Karol Bagh"       "New Delhi" "Delhi"      110005)
C5=$(new_client "Meera Boutique"        "hello@meeraboutique.example"   9856789012 "7 Hauz Khas Village" "New Delhi" "Delhi"      110016)
C6=$(new_client "Rajesh Cloth House"    "rajesh@clothhouse.example"     9867890123 "12 Johari Bazaar"    "Jaipur"    "Rajasthan"  302003)
C7=$(new_client "Sunrise Traders"       "orders@sunrisetraders.example" 9878901234 "44 CG Road"          "Ahmedabad" "Gujarat"    380009)
C8=$(new_client "Lakshmi Silks"         "billing@lakshmisilks.example"  9889012345 "9 Charminar Road"    "Hyderabad" "Telangana"  500002)
echo "Clients: $C1 $C2 $C3 $C4 $C5 $C6 $C7 $C8"

echo ""
echo "Creating items ..."
# Tax rates span 5%, 12% and 18% so the GST report's HSN summary has more than one row.
...
# The items endpoint returns rows sorted by name, not by id, so "the nth id in the
# response" is not "the nth item created". Look each one up by name instead.
ITEMS_JSON=$(auth "$BASE/items/$COMPANY_ID/all")
item_id() {
...
I_COTTON=$(item_id "Cotton Fabric Roll");    I_LINEN=$(item_id "Linen Fabric Roll")
I_DENIM=$(item_id "Denim Fabric Roll");      I_BANARASI=$(item_id "Banarasi Silk Saree")
I_KANJI=$(item_id "Kanjivaram Silk Saree");  I_HANDLOOM=$(item_id "Cotton Handloom Saree")
I_KURTA=$(item_id "Cotton Kurta (Men)");     I_SALWAR=$(item_id "Ladies Salwar Suit")
I_SHIRT=$(item_id "Formal Shirt (Men)");     I_DUPATTA=$(item_id "Silk Dupatta")
I_BELT=$(item_id "Leather Belt")
echo "Items resolved."

echo ""
echo "Creating invoices ..."
# The create response is {"invoice_id": N, ...} — not "id".
new_invoice() { auth -X POST "$BASE/invoices" -d "$1" | grep -o '"invoice_id":[0-9]*' | grep -o '[0-9]*'; }

INV1=$(new_invoice "{\"company_id\":$COMPANY_ID,\"client_id\":$C1,\"invoice_date\":\"$(d 2)\",\"due_date\":\"$(dplus 13)\",\"discount\":0,\"items\":[{\"item_id\":$I_COTTON,\"qty\":60,\"rate\":180,\"discount\":0,\"tax_rate\":5}]}")
INV2=$(new_invoice "{\"company_id\":$COMPANY_ID,\"client_id\":$C4,\"invoice_date\":\"$(d 3)\",\"due_date\":\"$(dplus 12)\",\"discount\":0,\"items\":[{\"item_id\":$I_BANARASI,\"qty\":6,\"rate\":3500,\"discount\":0,\"tax_rate\":12}]}")
INV3=$(new_invoice "{\"company_id\":$COMPANY_ID,\"client_id\":$C5,\"invoice_date\":\"$(d 5)\",\"due_date\":\"$(dplus 10)\",\"discount\":0,\"items\":[{\"item_id\":$I_SALWAR,\"qty\":18,\"rate\":1150,\"discount\":0,\"tax_rate\":5},{\"item_id\":$I_DUPATTA,\"qty\":30,\"rate\":340,\"discount\":0,\"tax_rate\":5}]}")
INV4=$(new_invoice "{\"company_id\":$COMPANY_ID,\"client_id\":$C2,\"invoice_date\":\"$(d 9)\",\"due_date\":\"$(dplus 6)\",\"discount\":0,\"items\":[{\"item_id\":$I_KANJI,\"qty\":4,\"rate\":6250,\"discount\":0,\"tax_rate\":12}]}")
INV5=$(new_invoice "{\"company_id\":$COMPANY_ID,\"client_id\":$C7,\"invoice_date\":\"$(d 14)\",\"due_date\":\"$(dplus 1)\",\"discount\":0,\"items\":[{\"item_id\":$I_SHIRT,\"qty\":22,\"rate\":899,\"discount\":0,\"tax_rate\":12},{\"item_id\":$I_BELT,\"qty\":15,\"rate\":599,\"discount\":0,\"tax_rate\":18}]}")
INV6=$(new_invoice "{\"company_id\":$COMPANY_ID,\"client_id\":$C3,\"invoice_date\":\"$(d 40)\",\"due_date\":\"$(d 12)\",\"discount\":0,\"items\":[{\"item_id\":$I_LINEN,\"qty\":45,\"rate\":395,\"discount\":0,\"tax_rate\":5}]}")
INV7=$(new_invoice "{\"company_id\":$COMPANY_ID,\"client_id\":$C6,\"invoice_date\":\"$(d 55)\",\"due_date\":\"$(d 25)\",\"discount\":0,\"items\":[{\"item_id\":$I_HANDLOOM,\"qty\":20,\"rate\":1050,\"discount\":0,\"tax_rate\":5}]}")
INV8=$(new_invoice "{\"company_id\":$COMPANY_ID,\"client_id\":$C8,\"invoice_date\":\"$(d 70)\",\"due_date\":\"$(d 40)\",\"discount\":0,\"items\":[{\"item_id\":$I_DENIM,\"qty\":28,\"rate\":265,\"discount\":0,\"tax_rate\":5}]}")
new_invoice "{\"company_id\":$COMPANY_ID,\"client_id\":$C1,\"invoice_date\":\"$(d 0)\",\"due_date\":\"$(dplus 15)\",\"discount\":0,\"items\":[{\"item_id\":$I_KURTA,\"qty\":10,\"rate\":550,\"discount\":0,\"tax_rate\":5}]}" >/dev/null
new_invoice "{\"company_id\":$COMPANY_ID,\"client_id\":$C5,\"invoice_date\":\"$(d 1)\",\"due_date\":\"$(dplus 14)\",\"discount\":0,\"items\":[{\"item_id\":$I_DUPATTA,\"qty\":12,\"rate\":340,\"discount\":0,\"tax_rate\":5}]}" >/dev/null
...
# Amounts are the exact invoice totals, so "paid" invoices settle to zero rather than
# leaving odd balances on screen.
pay() { auth -X POST "$BASE/payments" -d "$1" >/dev/null; }
pay "{\"client_id\":$C1,\"amount\":11340,\"payment_method\":\"bank_transfer\",\"reference\":\"UTR203948\",\"notes\":\"Full payment\"}"
pay "{\"client_id\":$C4,\"amount\":23520,\"payment_method\":\"bank_transfer\",\"reference\":\"UTR778812\",\"notes\":\"Full payment\"}"
pay "{\"client_id\":$C5,\"amount\":16222.50,\"payment_method\":\"upi\",\"reference\":\"UPI118034\",\"notes\":\"Part payment\"}"
pay "{\"client_id\":$C2,\"amount\":11200,\"payment_method\":\"upi\",\"reference\":\"UPI889931\",\"notes\":\"Part payment\"}"
...
auth -X POST "$BASE/estimates" -d "{\"company_id\":$COMPANY_ID,\"client_id\":$C3,\"estimate_date\":\"$(d 2)\",\"expiry_date\":\"$(dplus 18)\",\"discount\":0,\"items\":[{\"item_id\":$I_BANARASI,\"qty\":8,\"rate\":3500,\"discount\":0,\"tax_rate\":12}]}" >/dev/null
auth -X POST "$BASE/estimates" -d "{\"company_id\":$COMPANY_ID,\"client_id\":$C6,\"estimate_date\":\"$(d 7)\",\"expiry_date\":\"$(dplus 8)\",\"discount\":0,\"items\":[{\"item_id\":$I_SALWAR,\"qty\":25,\"rate\":1150,\"discount\":0,\"tax_rate\":5},{\"item_id\":$I_DUPATTA,\"qty\":40,\"rate\":340,\"discount\":0,\"tax_rate\":5}]}" >/dev/null
...
echo "You should see: 8 clients, 12 items in 4 categories, 10 invoices"
echo "(2 paid, 2 part-paid, 3 overdue, 2 draft), 4 payments, 5 expenses, 2 estimates."
